...
void cImage::Render(HDC hdc, int destX, int destY, int sourX, int sourY, int destWidth, int destHeight, int sourWidth, int sourHeight)
{
	if (m_isTrans)	// 배경색을 없앨 경우
	{
		// GdiTransparentBlt : 비트맵을 불러올때 특정색상을 제외하고 복사를 하는 함수
		GdiTransparentBlt(
			...
			destX,					// 복사 될 좌표 시작 지점 X
			destY,					// 복사 될 좌표 시작 지점 Y
			destWidth,				// 복사 될 이미지의 가로 크기
			destHeight,				// 복사 될 이미지의 세로 크기
			m_pImageInfo->hMemDC,	// 복사 할 대상 DC
			sourX,					// 복사 시작 지점 X
			sourY,					// 복사 시작 지점 Y
			sourWidth,				// 복사 영역 가로 크기
...
void cImage::AlphaRender(HDC hdc, int destX, int destY, int frameX, int frameY, BYTE alpha)
{
	// 알파블렌드 처음 사용시 초기화
	if (!m_pBlendImage) SetupForAlphaBlend();
	...
	{
		//1. 출력해야 될 DC에 그려져 있는 내용을 블렌드이미지에 그려준다
		BitBlt(m_pBlendImage->hMemDC, destX, destY, frameX, frameY,
			hdc, destX, destY, SRCCOPY);
		//2. 출력해야 될 이미지를 블렌드에 그려준다(마젠타 값을 없애준다)
		GdiTransparentBlt(m_pBlendImage->hMemDC, destX, destY, frameX, frameY,
			m_pImageInfo->hMemDC, 0, 0, m_pImageInfo->nWidth, m_pImageInfo->nHeight, m_transColor);
		//3. 블렌드 DC를 출력해야 할 DC에 그린다
		GdiAlphaBlend(hdc, destX, destY, frameX, frameY,
			m_pBlendImage->hMemDC, destX, destY, frameX, frameY, m_stBlendFunc);
	}
	else
	{
		// 알파블렌드 옵션 값을 사용해서 그린다.
...
void cImage::FrameRender(HDC hdc, int destX, int destY, int frameX, int frameY)
{
	if (m_isTrans)	// 배경색을 없앨 경우
	{
		// GdiTransparentBlt : 비트맵을 불러올때 특정색상을 제외하고 복사를 하는 함수
		GdiTransparentBlt(
			...
			m_pImageInfo->nFrameHeight, // 복사 될 이미지의 세로 크기
			m_pImageInfo->hMemDC,	// 복사 할 대상 DC
			frameX * m_pImageInfo->nFrameWidth, // 현재 프레임의 시작지점 X
			frameY * m_pImageInfo->nFrameHeight,// 현재 프레임의 시작지점 Y
			m_pImageInfo->nFrameWidth,	// 복사 영역 가로 크기
			m_pImageInfo->nFrameHeight,	// 복사 영역 세로 크기
			m_transColor			// 복사 할 때 제외 할 색상(투명처리)
		);
	...
	else // 원본 이미지 그대로 출력
	{
		...
		BitBlt(hdc, destX, destY,
			...
			m_pImageInfo->nFrameHeight,
			m_pImageInfo->hMemDC,
			frameX * m_pImageInfo->nFrameWidth,
			frameY * m_pImageInfo->nFrameHeight,
			SRCCOPY);
	}
}
