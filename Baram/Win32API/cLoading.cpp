...
void cLoadItem::InitForSound(string key, const char * fileName, bool bgm, bool loop)
{
	...
	m_stImageResource.key = string(key);
	m_stImageResource.fileName = fileName;
	m_stImageResource.isBgm = bgm;
	m_stImageResource.isLoop = loop;
}
...
void cLoading::Setup()
{
	m_pImgBackground = g_pImageManager->AddImage("Loading", "images/로딩.bmp", WINSIZEX, WINSIZEY);

	g_pImageManager->AddImage("BarBack", "images/로딩바맥스.bmp", 50, 10);
	g_pImageManager->AddImage("BarFront", "images/로딩바 위.bmp", 50, 10);

	// 로딩바 초기화
	m_pLoadingBar = new cProgressBar("BarBack", "BarFront", WINSIZEX - 100, 30);
	m_pLoadingBar->SetPosX(WINSIZEX / 2);
	m_pLoadingBar->SetPosY(WINSIZEY - 50);
	// 로딩 게이지 초기화
	m_nCurrentGauge = 0;
...
void cLoading::Render()
{
	...
	HFONT hFont = CreateFont(15, 0, 0, 0, FW_LIGHT, 0, 0, 0, HANGEUL_CHARSET,
		0, 0, 0, 0, 0);
...
bool cLoading::LoadingDone()
{
	...
	case LOAD_KIND_SOUND:
		img = item->GetImageResource();
		g_pSoundManager->AddSound(img.key, img.fileName, img.isBgm, img.isLoop);
		break;
	}
