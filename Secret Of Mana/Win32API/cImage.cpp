...
void cImage::Render(HDC hdc, int destX, int destY, int destWidth, int destHeight, int sourX, int sourY, int sourWidth, int sourHeight)
{
	//if (m_isTrans)	// 배경색을 없앨 경우
	//{
		// GdiTransparentBlt : 비트맵을 불러올때 특정색상을 제외하고 복사를 하는 함수
...
void cImage::AlphaRender(HDC hdc, int destX, int destY, int sourX, int sourY, BYTE alpha)
{
	...
	if (m_isTrans)
	{
		...
		BitBlt(m_pBlendImage->hMemDC, 0, 0, m_pImageInfo->nFrameWidth, m_pImageInfo->nFrameHeight,
			hdc, destX, destY, SRCCOPY);
		//2. 출력해야 될 이미지를 블렌드에 그려준다(마젠타 값을 없애준다)
		GdiTransparentBlt(m_pBlendImage->hMemDC, 0, 0, m_pImageInfo->nFrameWidth, m_pImageInfo->nFrameHeight,
			m_pImageInfo->hMemDC, sourX * m_pImageInfo->nFrameWidth, sourY *m_pImageInfo->nFrameHeight, m_pImageInfo->nFrameWidth, m_pImageInfo->nFrameHeight, m_transColor);
		//3. 블렌드 DC를 출력해야 할 DC에 그린다
...
void cImage::ViewPortRender(HDC hdc, RECT ViewPort, int x, int y, int MultX, int MultY)
{
	GdiTransparentBlt(hdc,
		0,
		0,
		...
		ViewPort.left,
		ViewPort.top,
		(ViewPort.right - ViewPort.left)/MultX,
		(ViewPort.bottom - ViewPort.top)/MultY,
		RGB(255, 0, 255));
}